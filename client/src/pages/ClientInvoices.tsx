...
export default function ClientInvoices() {
  ...
  const invoices = store.getClientInvoices();
        ...
        <div>
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            <Link href="/batch-invoice">
              <Button className="gap-2"><FileText className="w-4 h-4" /> New Invoice</Button>
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <Card>
            <CardContent className="p-4">
              ...
              <div className="text-lg font-bold font-mono text-amber-600">{fmt(totalDraft)}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <div className="text-xs text-muted-foreground uppercase tracking-wider">Sent</div>
              <div className="text-lg font-bold font-mono text-blue-600">{fmt(totalSent)}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <div className="text-xs text-muted-foreground uppercase tracking-wider">Paid</div>
              <div className="text-lg font-bold font-mono text-emerald-600">{fmt(totalPaid)}</div>
            </CardContent>
          </Card>
        </div>
        ...
        {invoices.length === 0 ? (
          <Card>
            <CardContent className="p-10 text-center text-muted-foreground">
              ...
              <Link href="/batch-invoice">
                <Button variant="outline" className="mt-3 gap-2"><FileText className="w-4 h-4" /> Go to Batch Builder</Button>
              </Link>
            </CardContent>
          </Card>
        ) : (
              ...
              <Card key={inv.invoiceNumber}>
                <CardHeader className="pb-2 pt-3 px-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" className="gap-1 h-7 text-xs" onClick={() => { exportClientInvoiceToExcel(inv); toast.success("Excel downloaded"); }}>
                        <Download className="w-3 h-3" /> Excel
                      </Button>
                      {inv.status === "draft" && (
                        <Button variant="outline" size="sm" className="gap-1 h-7 text-xs" onClick={() => toast.info("Mark as sent — coming soon")}>
                          <Send className="w-3 h-3" /> Mark Sent
                        </Button>
                      )}
                      {inv.status === "sent" && (
                        <Button variant="outline" size="sm" className="gap-1 h-7 text-xs" onClick={() => toast.info("Mark as paid — coming soon")}>
                ...
                <CardContent className="px-4 pb-4">
                  <div className="text-xs text-muted-foreground mb-2">
                    Date: {inv.invoiceDate} · Period: {inv.period} · {inv.lines.length} containers
                  </div>
                  <div className="overflow-auto">
                    <table className="w-full text-xs">
                      ...
                      <tbody className="divide-y">
                        {inv.lines.map(l => (
                          <tr key={l.containerNumber} className="hover:bg-muted/30">
                            <td className="px-2 py-1.5 font-mono font-semibold">{l.containerNumber}</td>
